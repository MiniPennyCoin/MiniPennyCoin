...
const char * const BITCOIN_CONF_FILENAME = "MiniPennyCoin.conf";
const char * const BITCOIN_PID_FILENAME = "MiniPennyCoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "MiniPennyCoin" is a composite category enabling all MiniPennyCoin-related debug output
            if(ptrCategory->count(string("MiniPennyCoin"))) {
...
#else
    const char* pszModule = "MiniPennyCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MiniPennyCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MiniPennyCoinCore
    // Mac: ~/Library/Application Support/MiniPennyCoinCore
    // Unix: ~/.MiniPennyCoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MiniPennyCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MiniPennyCoinCore";
#else
    // Unix
    return pathRet / ".MiniPennyCoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty MiniPennyCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override MiniPennyCoin.conf
